...
export function ExploreLanding({
  ...
  variant: 'gallery' | 'profiles'
  normalizedCategory: string
  taskRoute: string
  ui: Ui
}) {
  const copy = exploreLandingCopy[variant]
  const heroImage =
    variant === 'gallery'
  ...
  const Icon = variant === 'gallery' ? ImageIcon : User
  const pillars = copy.pillars

  return (
    <>
      <section className="relative -mx-4 mb-14 overflow-hidden rounded-lg sm:-mx-6 lg:-mx-8">
        <img src={heroImage} alt="" className="absolute inset-0 h-full min-h-[320px] w-full object-cover md:min-h-[380px]" loading="eager" />
        ...
        <div className="relative z-10 px-6 py-14 md:px-10 md:py-16 lg:px-12">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              href={variant === 'gallery' ? '/register' : '/images'}
            ...
            <Link
              href={variant === 'gallery' ? '/profile' : '/register'}
      ...
      <section className="mb-6 grid gap-4 rounded-lg border border-border bg-muted/30 p-6 sm:grid-cols-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-foreground">Curated lanes</p>
            <p className={`mt-1 text-xs leading-relaxed ${ui.muted}`}>Large cards, calm spacing, and forest-and-gold accents match the homepage.</p>
          </div>
        </div>
        <div className="flex items-start gap-3">
          <Heart className="mt-0.5 h-5 w-5 text-primary" />
          <div>
            <p className="text-sm font-semibold text-foreground">Made for creators</p>
            <p className={`mt-1 text-xs leading-relaxed ${ui.muted}`}>Profiles and galleries share one visual language so trust stays consistent.</p>
          </div>
        </div>
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className={`mt-1 text-xs leading-relaxed ${ui.muted}`}>Filter by category or jump into search without leaving this surface.</p>
          </div>
        </div>
      </section>
    </>
