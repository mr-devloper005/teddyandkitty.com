...
import { ExploreLanding } from '@/components/marketing/explore-landing'
import { CommunityLanding } from '@/components/marketing/community-landing'
...
/** Unified page canvas — layout variants only change structure, not color system */
const variantShells = {
  'listing-directory': 'site-page',
  'listing-showcase': 'site-page',
  'article-editorial': 'site-page',
  'article-journal': 'site-page',
  'image-masonry': 'site-page',
  'image-portfolio': 'site-page',
  'profile-creator': 'site-page',
  'profile-business': 'site-page',
  'classified-bulletin': 'site-page',
  'classified-market': 'site-page',
  'sbm-curation': 'site-page',
  'sbm-library': 'site-page',
} as const
...
export async function TaskListPage({ task, category }: { task: TaskKey; category?: string }) {
  ...
  const listBasePath = taskConfig?.route ?? (task === 'social' ? '/community' : '/posts')
  const schemaItems = posts.slice(0, 10).map((post, index) => ({
    '@type': 'ListItem',
    position: index + 1,
    url: `${baseUrl}${listBasePath}/${post.slug}`,
    name: post.title,
  }))
  const { recipe } = getFactoryState()
  const layoutKey = recipe.taskLayouts[task as keyof typeof recipe.taskLayouts] || `${task}-${task === 'listing' ? 'directory' : 'editorial'}`
  const shellClass = variantShells[layoutKey as keyof typeof variantShells] || 'site-page'
      ...
      <main className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        {layoutKey === 'listing-directory' || layoutKey === 'listing-showcase' ? (
          <section className="mb-12 grid gap-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
            <div className={`rounded-lg p-7 ${ui.panel}`}>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <Link href={taskConfig?.route || '#'} className={`inline-flex items-center gap-2 rounded-lg px-5 py-3 text-sm font-semibold ${ui.button}`}>Explore results <ArrowRight className="h-4 w-4" /></Link>
                <Link href="/search" className={`inline-flex items-center gap-2 rounded-lg border border-border bg-background px-5 py-3 text-sm font-semibold text-foreground hover:bg-muted/60`}>Open search</Link>
              </div>
            </div>
            <form className={`grid gap-3 rounded-lg p-6 ${ui.soft}`} action={taskConfig?.route || '#'}>
              <div>
          ...
          <section className="mb-12 grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
            ...
            <div className={`rounded-lg p-6 ${ui.panel}`}>
              ...
              <form className="mt-5 flex items-center gap-3" action={taskConfig?.route || '#'}>
                <select name="category" defaultValue={normalizedCategory} className={`h-11 flex-1 rounded-lg px-3 text-sm ${ui.input}`}>
                  <option value="all">All categories</option>
                  {CATEGORY_OPTIONS.map((item) => (
                    <option key={item.slug} value={item.slug}>{item.name}</option>
                  ))}
                </select>
                <button type="submit" className={`h-11 rounded-lg px-4 text-sm font-medium ${ui.button}`}>Apply</button>
              </form>
            </div>
          </section>
        ) : null}
        ...
        {task === 'image' || task === 'profile' ? (
          <ExploreLanding
            variant={task === 'image' ? 'gallery' : 'profiles'}
            normalizedCategory={normalizedCategory}
            taskRoute={taskConfig?.route || (task === 'image' ? '/images' : '/profile')}
            ui={ui}
          />
        ) : null}

        {task === 'social' ? (
          <CommunityLanding normalizedCategory={normalizedCategory} taskRoute={listBasePath} ui={ui} />
        ) : null}
          ...
          <section className="mb-12 grid gap-4 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {['Quick to scan', 'Shorter response path', 'Clearer urgency cues'].map((item) => (
                <div key={item} className={`rounded-lg p-5 ${ui.soft}`}>
        ...
        {intro && task !== 'image' && task !== 'profile' && task !== 'social' ? (
          <section className={`mb-12 rounded-lg p-6 sm:p-8 ${ui.panel}`}>
